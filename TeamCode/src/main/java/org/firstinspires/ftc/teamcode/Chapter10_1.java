...
public class Chapter10_1 extends OpMode { // defines our class extending OpMode for iterative execution
    ...
    public void loop() { // called repeatedly while the OpMode is running
        telemetry.addData("Amount blue", board.getAmountBlue()); // displays the blue color intensity from color sensor
        telemetry.addData("Distance (CM)", board.getDistance(DistanceUnit.CM)); // displays distance in centimeters
        telemetry.addData("Distance (IN)", board.getDistance(DistanceUnit.INCH)); // displays distance in inches
    }
}
